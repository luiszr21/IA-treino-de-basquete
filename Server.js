...
app.post("/api/llm", async (req, res) => {
  try {
          ...
          messages: [
            {
              role: "system",
              content:
                "Voce e um treinador de basquete analise a posição do jogador e a melhoria que ele deseja fazer e sugira treinos para que ele possa atingir seu objetivo na quadra.",
            },
            {
              role: "user",
              content: prompt,
    ...
    if (!text) {
      return res
        .status(502)
        .json({ erro: "Resposta vazia ou inesperada." });
    }
